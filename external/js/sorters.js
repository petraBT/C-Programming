...
class SIMSorters_QuickSorter extends SIMSorters_Sorter {
	// Lomuto partitioning, pivoting on the LAST element of the (sub)array being partitioned --
	// this matches the standard textbook C implementation:
	//   pivot = list[end];
	//   i = start - 1;
	//   for (j=start; j<end; j++) if (list[j] < pivot) { i++; swap(list[i], list[j]); }
	//   i++; swap(list[i], list[end]);
	// Optional pivot-selection strategies (see options below) swap a different element into the
	// "end" position before the scan above runs, so the scan itself is always the same.
	constructor(startingArray, barHeight, options, labels, name, color) {
		let backwards = options.has("REV") || options.has("BAC");
		...
		if (!name) name = SIMSorters_Sorter.buildDefaultName("Quicksort", options.has("UNL"), [backwards, "Reversed"], [shuffle, "Initial Shuffling"], [randomPivot, "Random Pivot"], [medianPivot, "Median Pivot"], [nondeterministic, "Nondeterministic"]);
	...
	// record the pre-step value of a bar's color the first time it's touched during the
	// current step, so undoStep() can put it back exactly (used by the tracking wrappers below)
	_uColorBar(index, c) {
		if (index >= 0 && index < this.bars.length && !(index in this._frame.colors)) {
			this._frame.colors[index] = this.bars[index].style.backgroundColor;
		}
		this.colorBar(index, c);
	}

	_uSwapBars(i, j) {
		this._frame.swap = [i, j];
		this.swapBars(i, j);
	}

	_uUnghostBars(i0, i1) {
		for (let k = i0; k < i1; k++) {
			if (k >= 0 && k < this.bars.length && !(k in this._frame.opac)) this._frame.opac[k] = this.bars[k].style.opacity;
		}
		this.unghostBars(i0, i1);
	}

	// read the (start, end) ranges still waiting on the stack, without popping them
	_stackRanges() {
		return this.xStack.map(packed => [(packed & 8191) - 4096, ((packed >> 13) & 8191) - 4096]);
	}

	doAlgoStep() {
		if (this._done) return true;

		// snapshot everything needed to undo this step exactly: the algorithm's own state,
		// the pending-range stack, and (via the _u* wrappers below) any bars whose color,
		// opacity, or position gets changed along the way
		this._frame = {
			mode: this.mode, start: this.start, end: this.end, i: this.i, j: this.j,
			low: this.low, high: this.high, stack: this.xStack.slice(), linger: this._lastLinger,
			colors: {}, opac: {}, swap: null
		};

		let finished = this._doAlgoStepInner();
		this.history.push(this._frame);
		return finished;
	}
	...
	_doAlgoStepInner() {
		...
		// POP THE NEXT (start, end) RANGE TO PARTITION OFF THE STACK
		case 0:
			...
			[this.start, this.end] = this.popFromStack(2);
			this._uUnghostBars(this.start, this.end + 1);

			// ranges of 0 or 1 elements are already sorted -- nothing to partition
			if (this.end <= this.start) {
				if (this.end === this.start) this._uColorBar(this.start, 1);
				this.mode = 0;
				return false;
			}

			if (this.MEDIAN_PIVOT) this.mode = 6;
			else if (this.RANDOM_PIVOT) this.mode = 7;
			else this.mode = 1;
			return false;

			// START A NEW PARTITION: pivot is the last element of [start, end]
		case 1:
			this.i = this.start - 1;
			this.j = this.start;
			this._uColorBar(this.end, 5);
			this._lastLinger = null;
			this.mode = 2;
			return false;

			// SCAN STEP: compare array[j] against the pivot (array[end])
		case 2:
			// clear the highlight left over from the previous comparison in this scan --
			// otherwise every bar we've ever compared stays lit up forever
			if (this.j > this.start) this._uColorBar(this.j - 1, 3);

			if (this.j < this.end) {
				this._uColorBar(this.j, 4);
				if (this.array[this.j] < this.array[this.end]) {
					this.i++;
					if (this.i !== this.j) this._uSwapBars(this.i, this.j);
					this._uColorBar(this.i, 3);
					this._lastLinger = null;
				}
				else this._lastLinger = this.j;
				this.j++;
				return false;
			}
			this.mode = 3;
			...
			// SWAP THE PIVOT INTO ITS FINAL POSITION, THEN RECURSE ON BOTH HALVES
		case 3:
			this.i++;
			this._uSwapBars(this.i, this.end);
			this._uColorBar(this.i, 1);

			// right half: (i+1, end)
			if (this.i + 1 < this.end) this.pushOntoStack(this.i + 1, this.end);
			else if (this.i + 1 === this.end) this._uColorBar(this.i + 1, 1);

			// left half: (start, i-1)
			if (this.start < this.i - 1) this.pushOntoStack(this.start, this.i - 1);
			else if (this.start === this.i - 1) this._uColorBar(this.start, 1);
			...
			// INITIAL F-Y SHUFFLE
		case 5:
			this._uColorBar(this.high, 3);
			this._uColorBar(this.low, 3);
			...
			this._uColorBar(this.low, 5);
			this._uColorBar(this.high, 5);
			this._uSwapBars(this.low, this.high);
			...
			// MEDIAN-OF-THREE: swap the median of array[start], array[mid], array[end]
			// into the end (pivot) position, then fall through to the normal scan
		case 6: {
			let mid = this.start + Math.floor((this.end - this.start) / 2);
			this._uColorBar(this.start, 4);
			this._uColorBar(mid, 4);
			this._uColorBar(this.end, 4);

			let a = this.array[this.start], b = this.array[mid], c = this.array[this.end];
			let medianIndex;
			if ((a >= b) === (a <= c)) medianIndex = this.start;
			else if ((b >= a) === (b <= c)) medianIndex = mid;
			else medianIndex = this.end;

			if (medianIndex !== this.end) this._uSwapBars(medianIndex, this.end);

			// only the pivot (now at `end`) stays highlighted -- mode 1 recolors it next;
			// clear the other two comparison bars so they don't stay stuck lit up
			this._uColorBar(this.start, 3);
			this._uColorBar(mid, 3);

			this.mode = 1;
			return false;
		}

			// RANDOM PIVOT: swap a uniformly random element of [start, end] into the
			// end (pivot) position, then fall through to the normal scan
		case 7: {
			let range = this.end - this.start + 1;
			let r;
			if (this.NONDETERMINISTIC) r = this.start + Math.floor(Math.random() * range);
			else r = this.start + (SIMSorters.mulberry32(this.RAND + this.start * 7919 + this.end) % range);

			if (r !== this.end) this._uSwapBars(r, this.end);
			this.mode = 1;
			return false;
		}
		}
	...
	undoStep(lastState) {
		...
		// undo any bar swap that happened during this step (swapping the same two bars
		// again is its own inverse -- it restores values, heights, labels, and colors alike)
		if (lastState.swap) this.swapBars(lastState.swap[0], lastState.swap[1]);

		// put back any bar colors/opacities this step changed (this also cleans up the
		// solid "all done" flourish painted after the final step, when undoing back out of it)
		for (let idx in lastState.colors) this.bars[idx].style.backgroundColor = lastState.colors[idx];
		for (let idx in lastState.opac) this.bars[idx].style.opacity = lastState.opac[idx];

		// restore the algorithm's own state to exactly how it was before this step ran
		this.mode = lastState.mode;
		this.start = lastState.start;
		this.end = lastState.end;
		this.i = lastState.i;
		this.j = lastState.j;
		this.low = lastState.low;
		this.high = lastState.high;
		this.xStack = lastState.stack.slice();
		this._lastLinger = lastState.linger;

		// repaint: everything not currently pending is finished, everything still on the
		// stack (or in the range we're about to work on) is pending. The pivot only gets
		// its highlight once mode 1 has actually run (modes 2/3), matching what the real
		// forward run would have shown at this exact point.
		this.colorBars(0, this.array.length, 1);
		for (let range of this._stackRanges()) this.colorBars(range[0], range[1] + 1, 3);
		if (this.end > this.start && (this.mode >= 1 && this.mode <= 3 || this.mode === 5 || this.mode === 6 || this.mode === 7)) {
			this.colorBars(this.start, this.end + 1, 3);
			if (this.mode === 2 || this.mode === 3) this.colorBar(this.end, 5);

			// median-of-three (mode 6) always leaves the chosen pivot highlighted at
			// color 4 -- it's only bumped up to color 5 once mode 1 actually runs
			else if (this.mode === 1 && this.MEDIAN_PIVOT) this.colorBar(this.end, 4);

			// if we're mid-scan and the previous comparison didn't find a swap, that bar
			// is still genuinely highlighted at this point (its cleanup happens at the
			// start of the *next* scan step, not before it)
			if (this.mode === 2 && this._lastLinger !== null && this._lastLinger !== undefined) {
				this.colorBar(this._lastLinger, 4);
			}
		}
	}
	...
	resetVars() {
		...
		this._lastLinger = null;
		// scratch variables used only by the INITIAL F-Y SHUFFLE step (mode 5)
		this.pivot = this.low = 0;
		this.highBound = this.high = this.array.length;

		// arrays of 0 or 1 elements are already sorted -- nothing to partition
		if (this.end <= this.start) {
			this.mode = -1;
			this._done = true;
		}
		else if (this.SHUFFLE) this.mode = 5;
